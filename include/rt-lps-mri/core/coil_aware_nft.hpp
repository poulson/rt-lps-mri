...
inline void
CoilAwareNFT2D
( int numTimesteps, int N0, int N1, int M, 
        ...
        DistMatrix<Complex<double>,STAR,VR>& F )
{
    ...
    for( int jLoc=0; jLoc<locWidth; ++jLoc )
    {
        ...
        {
            Complex<double>* FCol = F.Buffer(t*M,jLoc);
            const Complex<double>* FHatCol = FHat.LockedBuffer(t*N0*N1,jLoc);
  ...
  const DistMatrix<Complex<double>,STAR,VR>& F )
{
    ...
    for( int jLoc=0; jLoc<locWidth; ++jLoc )
    {
        ...
        for( int t=0; t<numTimesteps; ++t )
        {
            const Complex<double>* FCol = F.LockedBuffer(t*M,jLoc);
            Complex<double>* FHatCol = FHat.Buffer(t*N0*N1,jLoc);
            for( int xi=0; xi<M; ++xi )
            {
                const double x0 = XCol[2*xi+0];
                const double x1 = XCol[2*xi+1];
                ...
                for( int ki=0; ki<N0; ++ki )
                {
                    ...
                    for( int kj=0; kj<N1; ++kj )
                    {
                        ...
                        const double imagPart = sin(theta);
                        FHatCol[kj+ki*N1] += 
                            Complex<double>(realPart,imagPart)*f;
                    }
                }
            }
        }
